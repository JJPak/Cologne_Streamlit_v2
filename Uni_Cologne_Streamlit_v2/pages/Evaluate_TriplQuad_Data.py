...
# ---------------------------------------------------------------- Introduction
st.title(' Evaluate TQ data ')
st.subheader('A constant method for TQ evaluation at the UoC')

text_1 = """ If you just finished you TQ trace element measurement and already looked at the calibrations
...
TQ data and evaluate it as recommended by the AG Geo- and Cosmochemistry of the University of Cologne."""
st.markdown(text_1)
...
st.markdown(text_2)
# image1 = Image.open(path+'/images/TQ_eval_plot.png')
st.image(path+'/images/TQ_eval_plot.png')
...
upload_excel = st.file_uploader(
    'XLS / XLSX file uploader', type=['xlsx', 'xls', 'csv'])
st.markdown(input_file_marker_1, unsafe_allow_html=True)  # input field marker
...
of the TQ from sticky elements. These HNO3 measurements can as well be used to calculate the Background."""
st.markdown(text_4)
...
within low errors (~5%) equal to XRF-measurements, it shows that TQ soution measurements are not ideal to measure major element concentrations.
...
text14 = """ The concentrations of the TQ measurements are corrected to the intensity of the internal standard that you added to the sample. During intensity drift
of the TQ the internal standard elements and the analyte elements are shifted by the same amount. Therefore even though the total intensities changed relative to the 
    ...
    if upload_excel == None:
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            ...
            st.download_button(
                ...
                data=buffer,
                file_name="Evaluated_TQ_data.xlsx"
            )        
...
# MAKE SURE THAT BEFORE YOU ADD THE LARGE STO#ED6363 DATASET TO THE SMALL DATASET TO CALCULATE THE TQ MEASUREMENTS DELETE COLUMN WITH THE NAME OF THE EXCEL FILE
